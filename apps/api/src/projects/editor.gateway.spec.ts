...
describe('EditorGateway broadcasts', () => {
  ...
  it('emitComment broadcasts to the asset room', () => {
    const { gateway } = build();
    const { emit, to } = serverOf(gateway);
    const comment = { id: 'c1', caseNo: 1, authorId: 'a', authorName: 'A', text: 'x', createdAt: 'x' };
  ...
  it('both are no-ops (never throw) with no socket server (worker context)', () => {
    ...
    expect(() => gateway.emitComment('a', { id: 'c', caseNo: 1, authorId: 'a', authorName: 'A', text: 'x', createdAt: 'x' })).not.toThrow();
  });
});
