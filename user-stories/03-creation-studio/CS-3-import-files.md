...
- **Drawing-source allowlist (2026-07-13)**: accept the known drawing-app formats above by **extension** (Clip Studio `.clip`, Krita `.kra`, Procreate `.procreate`, `.psd`/`.psb`, `.sai`/`.sai2`, `.xcf`, `.ase`/`.aseprite`, `.ai`, `.tiff`) with a permissive content-type (`application/octet-stream` and any format-specific type). Store as `dessin`, **no preview derivative** (like `.psd`); no magic-byte gate for these (proprietary/zip-shared magic makes it unreliable) — extension + size cap only. One shared constant (extend `ASSET_ALLOWED_*` in `packages/shared/src/media.ts`) so upload validation, the FE hint, and the drop-zone `accept` stay in sync.
