...
- **A4 pagination on overflow — true content reflow** (2026-07-14, item 24): when a case's content exceeds one A4 page height, the content **reflows onto successive bordered A4 sheets** with a real studio-tone gap between them — the block that would cross the boundary is pushed down to the top of the next sheet, so text never runs across the gutter. Implemented as a pure **VIEW** concern (`components/editor/richtext/pagination.ts`): a ProseMirror plugin measures each sheet's rendered blocks and lays out one bordered A4 frame per page (hard offset shadow, ink border) behind the editable content, pushing overflowing blocks past the boundary with **widget-decoration spacers**. It **never mutates the ProseMirror doc or the Yjs CRDT** (no history/CRDT ops from pagination), so the caret's document position is untouched — on Enter at a boundary the caret just moves down with its block onto the next sheet, and scroll stays stable. Works identically in Manga (per-case sheets) and Prose (single body) modes, and is responsive: the A4 scales down on narrow widths (more, shorter pages) with no horizontal overflow. **Known limit**: breaks happen at **block boundaries** only — a single block taller than one usable page (e.g. a giant pasted paragraph with no internal break point) can't be split and is left to span pages (enough frames are still drawn beneath it); splitting inside a block would need a line-level layout engine and is out of scope.
