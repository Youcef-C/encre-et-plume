...
- "Lancer un concours" button opens the contest modal (`data-contest-modal`), extended (user-specified 2026-07-05) to capture as complete a brief as possible. On-brand controls throughout ([[F-20]] rule: `OnBrandSelect`/`OnBrandCheckbox`, `GenreChip`/`GenreSuggestInput` for genres, never bare native selects/free-text-for-categorized-fields). Fields:
  ...
  - **Jury & évaluation**: the **critères de notation** (rubric) — defaulted by content type ([[AD-9]] list), editable; the **échelle** (e.g. 0–10); **vote du public** on/off (+ the jury/public weighting when on). **Réviseur·es**: appoint jury members by searching users ([[F-7]]) — **any role** (editor / maintainer / admin / regular user); appointment grants judging rights for this contest only (managed here or later in the [[AD-9]] judging view).
...
- [[AD-9]] — judging, rubric scoring, ranking, public-vote config, and reviewer roster.
- [[F-7]] — user search to appoint reviewers (any role) to a contest.
