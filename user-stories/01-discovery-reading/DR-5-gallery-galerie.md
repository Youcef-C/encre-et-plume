...
- **"Toutes les illustrations"** grid: masonry grid cards (title, artist, category, ♥) with "👁 Aperçu rapide" quick-preview overlay → opens detail [[DR-6]]. Its heading becomes **"Résultats pour &lt;termes&gt;"** when a search is active (title in « guillemets », hashtags as `#tag`, genres as their labels), so the results replace the trending feature rather than sitting under it. The preview's close « ✕ » control must not disturb the overlay layout (user note 2026-07-04).
