...
- **Galerie** ([[DR-5]]): a **freetext hashtag filter** as a **multi-tag token field** (no button — [[F-20]] rule): **Enter / comma / space** commit the current text as a normalized `#tag` chip, **Backspace on an empty input removes the last chip**, and each chip is individually removable. Multiple hashtags **narrow** the grid (AND / `hasEvery`). Clicking an illustration's hashtag lands here with that tag pre-filled. Distinct control from the title/artist `q` search.
...
## Backend
- **Gallery list** (`apps/api/src/gallery/gallery.service.ts`): add a multi-value **`tags[]`** query param to the `GalleryQuery` (repeated `tags=` entries, mirroring `genre`) — exact-token AND match `hashtags: { hasEvery: <normalized tags> }`. Normalize each server-side (lowercase, strip leading `#`, trim, length cap), drop empties, dedupe. Keep `q` (title/artist) as-is.
